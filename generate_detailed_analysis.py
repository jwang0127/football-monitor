...
def build_match(match: dict[str, Any], evidence: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "overheatAudit": {"detected": p["overheatDetected"],
                          ...
                          "marketEvidenceConflict": p["marketEvidenceConflict"],
                          "frameworkLeader": LABEL[p["frameworkLeader"]],
                          "frameworkGap": p["frameworkGap"],
                          "conflictAction": next((x for x in p["corrections"] if "三维证据冲突" in x), "盘口与三维主方向一致")},
...
def render(payload: dict[str, Any]) -> str:
    ...
    for row in payload["matches"]:
        ...
        lines += [f"## {row['matchNumStr']} {f['home']} vs {f['away']}", "",
                  ...
                  f"- 盘口—三维冲突：{'是' if row['overheatAudit']['marketEvidenceConflict'] else '否'}；三维方向 {row['overheatAudit']['frameworkLeader']}，分差 {row['overheatAudit']['frameworkGap']}；处理：{row['overheatAudit']['conflictAction']}。", "",
                  "### 7. 综合结果", "",
